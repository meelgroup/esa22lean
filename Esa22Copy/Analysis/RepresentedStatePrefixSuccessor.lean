import Esa22Copy.Analysis.RepresentedStateOneStepCoupling
import Esa22Copy.Analysis.FreshSubsetRestrictionPattern

/-!
# Successor step for causal prefix representations

This module isolates the probabilistic successor step in the deferred-decisions
construction.  Given a causal coin-table representation through `r` arrivals, it
constructs one through `r + 1`: the next row supplies the refresh block, while an
injectively controlled, previously unused level frontier supplies the thinning subset.
-/

namespace Esa22Copy

/--
INTERNAL: one relaxed transition preserves the fixed-level representation and causal
coin-dependence invariant, with its thinning randomness supplied by a fresh frontier.
-/
theorem representedState_prefix_successor (P : Params) (A : Stream P)
    (i : Fin (P.m + 1))
  ...
  have hrm : r < P.m := by
    omega
  obtain ⟨representedState', hstep, hsample', hlevel', hcausal'⟩ :=
    representedState_one_step_coupling P A i hprefix hsucc hfrontier
      (freshSubset_restriction_pattern P) r hr
      representedState hsample hlevel hcausal
  refine ⟨representedState', ?_, hsample', hlevel', hcausal'⟩
...
* r7 · reduced · moved the exact one-step coupling and invariant preservation to
  `representedState_one_step_coupling`; closed the surrounding prefix-fold successor
* r4 · open · isolated the one-step deferred-decisions construction, including the
  exact old fiber invariant needed by `hfrontier`
-/

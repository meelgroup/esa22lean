...
theorem small_stream_accuracy (P : Params) (A : Stream P)
  ...
  have hstep : ∀ (a : Item P) (s : State P),
      ...
      step P a s = pure (State.mk (insert a s.samples) 0 none) := by
  ...
  have hfold : ∀ (l : List (Item P)) (s : State P),
    ...
    | cons a l ih =>
        ...
        let s₁ : State P := State.mk (insert a s.samples) 0 none
  ...
  have hfinish : finish t = some (F0 A) := by
    simp [finish, htanswer, rate, htlevel, hsampleFinal, F0]
  have hrun : run P A =
      pure (finish t, 0) := by
    rw [run, hrunState]
    change PMF.bind (PMF.pure t) (fun s =>
      PMF.pure (finish s, 0)) = _
    rw [PMF.pure_bind]
    rfl
  have hexact : (some (F0 A)) ∈ accurateEvent P A := by
    simp only [accurateEvent, Set.mem_setOf_eq, Arlib.relErr, Set.mem_Icc]
    have hF : 0 ≤ (F0 A : Real) := Nat.cast_nonneg _
    constructor <;> nlinarith [P.heps.1]
  have hout : finish t ∈ accurateEvent P A := by
    show finish t ∈ accurateEvent P A
    rw [hfinish]
    exact hexact
  rw [hrun]
  have hprob : Arlib.Approximation.outProbR
    ...
    have hmeasure :
        (PMF.pure (finish t, 0)).toOuterMeasure
